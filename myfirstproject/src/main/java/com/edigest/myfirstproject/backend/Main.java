...
public class Main {
    ...
    public static String main(String id, String source, String destination, String detourInput, String seatsInput) {
        ...
        if (isClient) {
            ...
            double detour;
            try {
                detour = detourInput.isEmpty() ? 10.0 : Double.parseDouble(detourInput);
                if (detour <= 0) detour = 10.0; // default minimum
        ...
        if (isDriver) {
            ...
            try {
                Thread.sleep(15000); // 15 sec wait for clients
    ...
    private static String assignClientsToDriver(RideOffer driver, WeightedGraph graph) {
        ...
        synchronized (lock) {
            ...
            rideRequests.clear(); // reset
        }

        result.append("\n📋 Ride Summary for Driver ").append(driver.getDriverId()).append(":\n");
        ...
        for (RideRequest req : optimalOrder) {
            ...
            result.append("    ").append(prev).append(" -> ").append(req.getSource())
                    .append(" : ").append(leg).append(" units\n");
            totalDistance += leg;
            prev = req.getSource();
        }
